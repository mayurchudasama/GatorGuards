...
public class DbHelperHG extends SQLiteOpenHelper {
	...
	public static final String KEY_GTOTAL_TIME= "total_time";
...
//					cursor.getString(cursor.getColumnIndex(KEY_RE_IMAGE_PATH))
	...
	public HashMap<String, String> getLastTimeINOUT()
	{
		...
		if(cursor.getCount()>0)
		{
			if (cursor.moveToFirst()) {
				do {
//					cursor.getString(cursor.getColumnIndex(KEY_RE_IMAGE_PATH))
					inOut.put("in",cursor.getString(cursor.getColumnIndex(KEY_GTIME_IN)));
					inOut.put("out",cursor.getString(cursor.getColumnIndex(KEY_GTIME_OUT)));
					return inOut;
				} while (cursor.moveToNext());
			}
			db.close();
	...
	public int insertAudioRecord(String id,String name,String location,String cost_codes,String time_in,String time_out,String total_time) {
		...
		int success=0;
			try
			{
				...
				values.put(KEY_GTOTAL_TIME, total_time);
	...
	public int insertTotalTime(String totalTime,String lastId) {
		...
		try
		{
			...
			SQLiteDatabase db = this.getReadableDatabase();
			ContentValues values = new ContentValues();
			values.put(KEY_GTOTAL_TIME, totalTime);
			db.update(TABLE_GATOR_GUARDS, values, KEY_AUTO_ID+"=?", new String[]{lastId});
