...
//					cursor.getString(cursor.getColumnIndex(KEY_RE_IMAGE_PATH))
	...
	public String getLastInTime(String id)
	{
		String query="SELECT "+KEY_GTIME_IN+" FROM "+TABLE_GATOR_GUARDS+" WHERE "+KEY_AUTO_ID+" = ?";
		SQLiteDatabase db = this.getReadableDatabase();
		...
		if(cursor.getCount()>0)
		{
			if (cursor.moveToFirst()) {
				do {
					in=cursor.getString(cursor.getColumnIndex(KEY_GTIME_IN));
					cursor.close();
				} while (cursor.moveToNext());
			}
			db.close();
	...
	public int insertTimeOut(String timeout,String lastId,String total_time) {
